...
function initialPage() {
    $(window).resize(function() {
        Dept.table.resetHeight({height: $(window).height()-100});
    });
}
var setting = {
    data: {
...
Dept.initColumn = function () {
    var columns = [
        ...
        {title: '上级部门', field: 'parentName', align: 'center', valign: 'middle', sortable: true, width: '300px'},
...
function getDeptId () {
    ...
    var selected =$('#dataGrid').bootstrapTreeTable('getSelections');// Dept.table.getSelectedRow();
    if (selected.length == 0) {
        alert("请选择一条记录");
        return false;
    } else {
        console.log("dept:"+selected[0].id);
